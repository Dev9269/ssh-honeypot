...
class TestDatabase:
    ...
    def setup_db(self):
        ...
        _old_log_dir = config.LOG_DIR
        _old_db_path = config.DB_PATH
        ...
        if hasattr(db, "_local"):
            try:
                ...
                pass
        config.LOG_DIR = _old_log_dir
        config.DB_PATH = _old_db_path
...
class TestLogger:
    ...
    def setup_logger(self):
        ...
        _old_json_log = config.JSON_LOG_FILE
        ...
        config.JSON_LOG_FILE = _old_json_log
        logger_mod.reset_logger()
    ...
    def test_log_with_mitre(self, setup_logger):
        ...
        log = logger.HoneypotLogger()
        log.log_attempt(
            "10.0.0.1", "admin", "admin123", "password", mitre="T1110:Brute Force"
        )
    ...
    def test_many_db_inserts(self):
        ...
        for i in range(500):
            db.insert_auth_attempt(
                f"10.0.0.{i % 50}", f"user{i}", f"pass{i}", "password"
            )
        stats = db.get_stats()
