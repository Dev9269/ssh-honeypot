...
class AlertManager:
    ...
    def send_alert(
        self, title: str, message: str, severity: str = "low", data: Dict = None
    ):
...
"""
            ...
            msg = MIMEText(body)
            msg["Subject"] = (
                f"[Honeypot] {payload['severity'].upper()} - {payload['title']}"
            )
            msg["From"] = config.ALERT_EMAIL_FROM
            msg["To"] = config.ALERT_EMAIL_TO

            with smtplib.SMTP(
                config.ALERT_EMAIL_SMTP, config.ALERT_EMAIL_PORT
            ) as server:
